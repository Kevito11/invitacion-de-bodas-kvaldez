...
const GuestInvitation: React.FC = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            // 2. Database Mode (Short Link via Supabase)
            if (id) {
                try {
                    const { getInvitationFromDb } = await import('../services/database');
                    const dbData = await getInvitationFromDb(id);
                    if (dbData) {
                        setData(dbData);
                        return;
                    } else {
                        alert("⚠️ Invitación no encontrada.");
                    }
                } catch (e) {
            ...
            // 3. Compressed Data Mode (Legacy Short Link)
